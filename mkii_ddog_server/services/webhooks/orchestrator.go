...
func toAlertEvent(event *WebhookEvent) *types.AlertEvent {
	return &types.AlertEvent{
		...
		Payload: types.AlertPayload{
			...
			SnapshotURL:         event.Payload.SnapshotURL,
			Link:                event.Payload.Link,
			OrgID:               event.Payload.OrgID,
			OrgName:             event.Payload.OrgName,
			AlertState:          event.Payload.AlertState,
			AlertTitleCustom:    event.Payload.AlertTitleCustom,
			ApplicationLongname: event.Payload.ApplicationLongname,
			ApplicationTeam:     event.Payload.ApplicationTeam,
			DetailedDescription: event.Payload.DetailedDescription,
			Impact:              event.Payload.Impact,
			Metric:              event.Payload.Metric,
			SupportGroup:        event.Payload.SupportGroup,
